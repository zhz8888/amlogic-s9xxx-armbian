...
| kernel_source    | unifreq       | Specifies the source code repository for compiling the kernel. You can choose the kernel source code repository from `github.com`, `gitlab.com`, `bitbucket.org`, `gitee.com` or custom Git servers. Default value is `unifreq`. Refer to `-r` for functionality. |
...
| kernel_toolchain | clang         | Sets the toolchain for compiling the kernel. Default value is `clang`. Refer to `-t` for functionality. |
